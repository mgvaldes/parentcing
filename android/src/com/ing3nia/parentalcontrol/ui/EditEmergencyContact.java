...
public class EditEmergencyContact extends Activity {
	...
	private EmergencyNumberModel editEmergencyContact;
	private boolean isAddedOrDeleted;
	...
	public void onCreate(Bundle savedInstanceState) {
	    ...
	    if (httpClientHandlerApp == null) { 
			httpClientHandlerApp = (HttpClientHandler) EditEmergencyContact.this.getApplication();
		}
	    ...
	    setContentView(R.layout.edit_emergency_contact);
	    ...
	    editEmergencyContact = getIntent().getBundleExtra("smartphonesInfo").getParcelable("emergencyContact");
	    isAddedOrDeleted = getIntent().getBundleExtra("smartphonesInfo").getBoolean("isAddedOrDeleted");
	    selectedContact = getIntent().getBundleExtra("smartphonesInfo").getInt("selectedContact");
	    
	    countrySpinner = (Spinner) findViewById(R.id.spinnerCountry);
	    countrySpinner.setSelection(getCountryPosition(editEmergencyContact.getCountry()));
	    
	    editTextPhoneNumber = (EditText) findViewById(R.id.editTextPhoneNumber);
	    editTextPhoneNumber.setText(editEmergencyContact.getNumber());
	    
	    editTextDescription = (EditText) findViewById(R.id.editTextDescription);
	    editTextDescription.setText(editEmergencyContact.getDescription());
	}
	...
	public int getCountryPosition(String country) {
		String[] countries = getResources().getStringArray(R.array.country_options);
	...
	public void loadDailyRoute() {
		Intent i = new Intent(EditEmergencyContact.this, DailyRoute.class);
		httpClientHandlerApp.lastView = PCViewsEnum.EDIT_RULE_VIEW.getId();
		startActivity(i);
	}

	public void loadAlertList() {
		Intent i = new Intent(EditEmergencyContact.this, AlertList.class);
		httpClientHandlerApp.lastView = PCViewsEnum.EDIT_RULE_VIEW.getId();
		startActivity(i);
	}
	
	public void loadRuleList() {
		Intent i = new Intent(EditEmergencyContact.this, RuleList.class);
		httpClientHandlerApp.lastView = PCViewsEnum.EDIT_RULE_VIEW.getId();
		startActivity(i);
	}
	
	public void loadContactList() {
		Intent i = new Intent(EditEmergencyContact.this, ContactList.class);
		httpClientHandlerApp.lastView = PCViewsEnum.EDIT_RULE_VIEW.getId();
		startActivity(i);
	}
	
	public void loadSmartphoneSettings() {
		Intent i = new Intent(EditEmergencyContact.this, SmartphoneSettings.class);
		httpClientHandlerApp.lastView = PCViewsEnum.EDIT_RULE_VIEW.getId();
		startActivity(i);
	}
	...
	public void loadDeviceMap() {
		if (httpClientHandlerApp.pendingChanges) {
			...
			Intent i = new Intent(EditEmergencyContact.this, SmartphoneMap.class);
			startActivity(i);
		}
	}
	...
	public void loadDeviceList() {
		if (httpClientHandlerApp.pendingChanges) {
			...
			Intent i = new Intent(EditEmergencyContact.this, SmartphoneList.class);
			startActivity(i);
		}
	}
	...
	public void saveChangesInSmartphone(EmergencyNumberModel emergencyContact) {
		EmergencyNumberModel changed;
		
		if (isAddedOrDeleted) {
			changed = this.smartphone.getAddedEmergencyNumbers().get(selectedContact);
		}
		else {
			changed = this.smartphone.getDeletedEmergencyNumbers().get(selectedContact);
		}
		
		changed.setCountry(emergencyContact.getCountry());
		changed.setDescription(emergencyContact.getDescription());
		changed.setNumber(emergencyContact.getNumber());
	}
	
	public void onSaveContactClick(View v) {
		if (checkEditTextFieldsFilled()) {
			...
			EmergencyNumberModel newEmergencyNumber = new EmergencyNumberModel(countries[countrySpinner.getSelectedItemPosition()], editTextPhoneNumber.getText().toString(), editTextDescription.getText().toString());
			newEmergencyNumber.setKeyId(editEmergencyContact.getKeyId());
			addedEmergencyContacts.add(newEmergencyNumber);
			
			///////////////////////////////////////////////
			// Agregar emergency contact en smartphone
			///////////////////////////////////////////////
			saveChangesInSmartphone(newEmergencyNumber);
			
			Intent i = new Intent(EditEmergencyContact.this, EmergencyContactList.class);
			
			httpClientHandlerApp.pendingChanges = true;
			httpClientHandlerApp.lastView = PCViewsEnum.EDIT_EMERGENCY_CONTACT_VIEW.getId();
			
			startActivity(i);
		}
		else {
			Toast.makeText(this, "All fields must be filled out!", Toast.LENGTH_SHORT);
	...
	protected Dialog onCreateDialog(int id) {
		if (id == ALERT_DIALOG) {
			AlertDialog.Builder alert = new AlertDialog.Builder(EditEmergencyContact.this);
	...
	private void showProcessDialog() {
		processDialog = ProgressDialog.show(EditEmergencyContact.this, null, EditEmergencyContact.this.getString(R.string.progress_dialog_save_changes), true, true,
			new OnCancelListener() {
				@Override
				public void onCancel(DialogInterface dialog) {
					if (asyncSaveChangesThread.cancel(true)) {
						UiUtils.levantarDialog(EditEmergencyContact.this, UiUtils.DIALOG_OK, getString(R.string.progress_dialog_save_changes_cancel));
					} 
					else {
	...
	private void showTicketsListProcessDialog() {
		processDialog = ProgressDialog.show(EditEmergencyContact.this, null, EditEmergencyContact.this.getString(R.string.progress_dialog_tickets_list), true, true,
			new OnCancelListener() {
				@Override
				public void onCancel(DialogInterface dialog) {
					if (asyncTicketsListThread.cancel(true)) {
						UiUtils.levantarDialog(EditEmergencyContact.this, UiUtils.DIALOG_OK, getString(R.string.progress_dialog_tickets_list_cancel));
					} 
					else {
	...
	private void showAdminUserListProcessDialog() {
		processDialog = ProgressDialog.show(EditEmergencyContact.this, null, EditEmergencyContact.this.getString(R.string.progress_dialog_admin_user_list), true, true,
			new OnCancelListener() {
				@Override
				public void onCancel(DialogInterface dialog) {
					if (asyncAdminUserListThread.cancel(true)) {
						UiUtils.levantarDialog(EditEmergencyContact.this, UiUtils.DIALOG_OK, getString(R.string.progress_dialog_admin_user_list_cancel));
					} 
					else {
			...
			try {
				...
				if (inputStream != null) {
					...
					if (code.equals("00")) {			
						...
						if (nextScreen == 0) {
							i = new Intent(EditEmergencyContact.this, SmartphoneMap.class);
						}
						else {
							i = new Intent(EditEmergencyContact.this, SmartphoneList.class);
						}
			...
			try {
				...
				if (inputStream != null) {
					...
					if (code.equals("00")) {
						...
						Intent i = new Intent(EditEmergencyContact.this, TicketList.class);
			...
			try {
				...
				if (inputStream != null) {
					...
					if (code.equals("00")) {
						...
						Intent i = new Intent(EditEmergencyContact.this, AdminUserList.class);
						httpClientHandlerApp.lastView = PCViewsEnum.DAILY_ROUTE_VIEW.getId();
