...
public class InternalModificationsResource {
	...
	private static Logger logger = Logger
			.getLogger(InternalModificationsResource.class.getName());
	 ...
	 * 'modification':{ 'inactiveContacts':[ { 'firstName':'Jamey',
	 * 'lastName”:'Jhonson', 'phones':[ {'type':1,'phoneNumber':'555432126'},
	 * {'type':1,'phoneNumber':'551989213'} ] } ], 'activeContacs':[ {
	 * 'firstName':'Maria', 'lastName':'Elena', 'phones':[
	 * {'type':1,'phoneNumber':'04267336220'},
	 * {'type':1,'phoneNumber':'623554123'} ] } ], 'deletedEmergencyNumbers':[
	...
	public Response newWS(String body) {
		...
		Type bodyType = new TypeToken<InternalModificationsModel>() {
		}.getType();
		...
		try {
			...
			logger.warning("[Internal Modifications Service] InternalModificationsModel couldn't be created from post input "
					+ WSStatus.INTERNAL_SERVICE_ERROR.getMsg());
			logger.severe("[Internal Modifications Service] Caugth exception: "
					+ e.getMessage() + " " + e);
		...
		try {
			...
			logger.warning("[Internal Modifications Service] An error ocurred while converting a Key to String. "
					+ ex.getMessage());
			...
			logger.severe("[Internal Modifications Service] An error ocurred while finding the PCSmartphone by key or adding contact. "
					+ ex.getMessage());
	...
	public void saveInternalModificationsNEW(
			InternalModificationsModel internalModsModel)
		...
		try {
			logger.info("[Internal Modifications Service - Cache Version] Converting id: "
					+ internalModsModel.getId() + " of smartphone to Key.");
			Key smartphoneKey = KeyFactory.stringToKey(internalModsModel
					.getId());

			logger.info("[Internal Modifications Service - Cache Version] Searching for smartphone in DB.");
			PCSmartphone savedSmartphone = pm.getObjectById(PCSmartphone.class,
					smartphoneKey);

			logger.info("[Internal Modifications Service - Cache Version] Searching for smartphone in cache.");
			String smartphoneCacheKey = SmartphoneCacheParams.SMARTPHONE
					+ internalModsModel.getId();
			IdentifiableValue cacheIdentSmartphone = (IdentifiableValue) syncCache
					.getIdentifiable(smartphoneCacheKey);
			SmartphoneCacheModel cacheSmartphone = null;

			if (cacheIdentSmartphone == null) {
				cacheSmartphone = WriteToCache.writeSmartphoneToCache(savedSmartphone);

				//cacheIdentSmartphone = (IdentifiableValue) syncCache
				//		.getIdentifiable(smartphoneCacheKey);
				//cacheSmartphone = (SmartphoneCacheModel) cacheIdentSmartphone
				//		.getValue();
			} else {
				cacheSmartphone = (SmartphoneCacheModel) cacheIdentSmartphone
						.getValue();
			...
			logger.info("[Internal Modifications Service - Cache Version] Setting location "
					+ internalModsModel.getLocation());

			if (internalModsModel.getLocation() != null) {
				GeoPt geopt = LocationModelUtils
						.convertToGeoPt(internalModsModel.getLocation());
				...
				String smartphoneRouteCacheKey = internalModsModel.getId()
						+ SmartphoneCacheParams.ROUTES;
				IdentifiableValue cacheIdentSmartphoneRoute = (IdentifiableValue) syncCache
						.getIdentifiable(smartphoneRouteCacheKey);
				...
				if (cacheIdentSmartphoneRoute == null) {
					...
					cacheSmartphoneRoutes = WriteToCache.writeSmartphoneRoutesToCache(
							internalModsModel.getId(), routeList);

					//cacheIdentSmartphoneRoute = syncCache
					//		.getIdentifiable(smartphoneRouteCacheKey);
					//cacheSmartphoneRoutes = (ArrayList<RouteModel>) cacheIdentSmartphoneRoute
					//		.getValue();
				} else {
					cacheSmartphoneRoutes = (ArrayList<RouteModel>) cacheIdentSmartphoneRoute
							.getValue();
				...
				if (savedSmartphone.getRoutes() != null) {
					...
					SimpleDateFormat dateFormatter = new SimpleDateFormat(
							"dd/HH/yyyy hh:mm:ss aa");

					if (savedSize > 0 && cacheSize > 0) {
						logger.info("[Internal Modifications Service - Cache Version] Obtaining last PCRoute");
						Key routeKey = savedSmartphone.getRoutes().get(
								savedSize - 1);
						PCRoute route = pm.getObjectById(PCRoute.class,
								routeKey);
						RouteModel cacheRoute = cacheSmartphoneRoutes
								.get(cacheSize - 1);

						Date currentDate = new Date();
						Date routeDate = route.getDate();

						Date cacheRouteDate = dateFormatter.parse(cacheRoute
								.getDate());
						...
						if (currentDay == routeDay
							...
							cacheNewRoute.setKeyId(KeyFactory
									.keyToString(newRoute.getKey()));
							cacheNewRoute.setDate(dateFormatter
									.format(new Date()));
							cacheNewRoute
									.setPoints(new ArrayList<LocationModel>());
							cacheNewRoute.getPoints().add(
									internalModsModel.getLocation());
						...
						RouteModel cacheNewRoute = new RouteModel();
						cacheNewRoute.setKeyId(KeyFactory.keyToString(newRoute
								.getKey()));
				...
				WriteToCache.writeSmartphoneRoutesToCache(
						cacheSmartphoneRoutes, internalModsModel.getId());
			}
			...
			PCModification savedModification = pm.getObjectById(
					PCModification.class, modificationKey);

			String smartphoneModificationCacheKey = internalModsModel.getId()
					+ SmartphoneCacheParams.MODIFICATION;
			IdentifiableValue cacheIdentSmartphoneModification = (IdentifiableValue) syncCache
					.getIdentifiable(smartphoneModificationCacheKey);
			ModificationModel cacheModification = null;

			if (cacheIdentSmartphoneModification == null) {
				cacheModification = WriteToCache.writeSmartphoneModificationToCache(
						internalModsModel.getId(), savedModification);

				//cacheIdentSmartphoneModification = syncCache
				//		.getIdentifiable(smartphoneModificationCacheKey);
				//cacheModification = (ModificationModel) cacheIdentSmartphoneModification
				//		.getValue();
			} else {
				cacheModification = (ModificationModel) cacheIdentSmartphoneModification
						.getValue();
			...
			if (internalModsModel.getModification().getInactiveContacts() == null) {
				internalModsModel.getModification().setInactiveContacts(
						new ArrayList<SimpleContactModel>());
			}

			if (internalModsModel.getModification().getAddedEmergencyNumbers() == null) {
				internalModsModel.getModification().setAddedEmergencyNumbers(
						new ArrayList<EmergencyNumberModel>());
			}

			if (internalModsModel.getModification()
					.getDeletedEmergencyNumbers() == null) {
				internalModsModel.getModification().setDeletedEmergencyNumbers(
						new ArrayList<EmergencyNumberModel>());
			}

			if (internalModsModel.getModification().getActiveContacts() != null
					&& internalModsModel.getModification()
							.getInactiveContacts() != null) {
...
//				ArrayList<PhoneModel> cacheInactiveContactsObjectList = new ArrayList<PhoneModel>();
				...
				IdentifiableValue cacheIdentSmartphoneActive = (IdentifiableValue) syncCache
						.getIdentifiable(smartphoneActiveCacheKey);
				ArrayList<SimpleContactModel> cacheSimpleActiveContactsObjectList = null;

				String smartphoneInactiveCacheKey = internalModsModel.getId()
						+ SmartphoneCacheParams.INACTIVE_CONTACTS;
				IdentifiableValue cacheIdentSmartphoneInactive = (IdentifiableValue) syncCache
						.getIdentifiable(smartphoneInactiveCacheKey);
				...
				if (internalModsModel.getModification().getActiveContacts()
					...
					if (cacheIdentSmartphoneActive == null) {
						...
						// cacheIdentSmartphoneActive = (IdentifiableValue)
						// syncCache.getIdentifiable(smartphoneActiveCacheKey);
						// cacheSimpleActiveContactsObjectList =
						// (ArrayList<SimpleContactModel>)
						// cacheIdentSmartphoneActive.getValue();
					} else {
						cacheSimpleActiveContactsObjectList = (ArrayList<SimpleContactModel>) cacheIdentSmartphoneActive
								.getValue();
					}
					...
					if (cacheIdentSmartphoneInactive == null) {
						...
						// cacheIdentSmartphoneInactive = (IdentifiableValue)
						// syncCache.getIdentifiable(smartphoneInactiveCacheKey);
						// cacheSimpleInactiveContactsObjectList =
						// (ArrayList<SimpleContactModel>)
						// cacheIdentSmartphoneInactive.getValue();
					} else {
						ArrayList<SimpleContactModel> value = (ArrayList<SimpleContactModel>) cacheIdentSmartphoneInactive
								.getValue();
...
//							cacheModification);
				...
				WriteToCache.writeSmartphoneActiveContactsToCache(
						internalModsModel.getId(),
						cacheSimpleActiveContactsObjectList);
				WriteToCache.writeSmartphoneInactiveContactsToCache(
						internalModsModel.getId(),
						cacheSimpleInactiveContactsObjectList);
			} else {
			...
			if (internalModsModel.getModification().getAddedEmergencyNumbers() != null
					&& internalModsModel.getModification()
							.getDeletedEmergencyNumbers() != null) {
				if (internalModsModel.getModification()
						.getDeletedEmergencyNumbers().size() > 0) {
					...
					ArrayList<Key> savedAddedEmergencyNumbers = savedSmartphone
							.getAddedEmergencyNumbers();

					for (Key key : savedAddedEmergencyNumbers) {
						auxAddedEmergencyNumber = (PCEmergencyNumber) pm
								.getObjectById(PCEmergencyNumber.class, key);
						addedEmergencyNumbers.add(auxAddedEmergencyNumber);
					}

					String smartphoneAddedEmergencyCacheKey = internalModsModel
							.getId()
							+ SmartphoneCacheParams.ADD_EMERGENCY_NUMBERS;
					IdentifiableValue cacheIdentSmartphoneAddedEmergency = (IdentifiableValue) syncCache
							.getIdentifiable(smartphoneAddedEmergencyCacheKey);
					ArrayList<EmergencyNumberModel> cacheAddedEmergencyNumberObjectList = null;

					if (cacheIdentSmartphoneAddedEmergency == null) {
						cacheAddedEmergencyNumberObjectList = WriteToCache.writeSmartphoneAddedEmergencyNumbersToCache(internalModsModel.getId(),addedEmergencyNumbers);

						//cacheIdentSmartphoneAddedEmergency = (IdentifiableValue) syncCache
						//		.getIdentifiable(smartphoneAddedEmergencyCacheKey);
						//cacheAddedEmergencyNumberObjectList = (ArrayList<EmergencyNumberModel>) cacheIdentSmartphoneAddedEmergency
						//		.getValue();
					} else {
						cacheAddedEmergencyNumberObjectList = (ArrayList<EmergencyNumberModel>) cacheIdentSmartphoneAddedEmergency
								.getValue();
					...
					checkDeletedEmergencyNumbersNEW(internalModsModel
							.getModification().getDeletedEmergencyNumbers(),
							addedEmergencyNumbers,
							cacheAddedEmergencyNumberObjectList,
							savedModification, cacheModification);

					WriteToCache.writeSmartphoneAddedEmergencyNumbersToCache(
							cacheAddedEmergencyNumberObjectList,
							internalModsModel.getId());
				}
			...
			WriteToCache.writeSmartphoneModificationToCache(
					internalModsModel.getId(), cacheModification);
		} catch (IllegalArgumentException ex) {
			logger.severe("[Internal Modifications Service - Cache Version] An error ocurred while finding the PCSmartphone by key "
					+ ex.getMessage());
			throw ex;
		} catch (NullPointerException ex) {
			logger.severe("[Internal Modifications Service - Cache Version] A null pointer exception ocurred "
					+ internalModsModel
					+ " "
					+ internalModsModel.getModification()
					+ " "
	...
	public void checkDeletedEmergencyNumbersNEW(
			ArrayList<EmergencyNumberModel> internalModDeletedEmergencyNumbers,
			ArrayList<PCEmergencyNumber> addedEmergencyNumbers,
			ArrayList<EmergencyNumberModel> cacheAddedEmergencyNumberObjectList,
		...
		for (EmergencyNumberModel em : internalModDeletedEmergencyNumbers) {
			checkInAddedEmergencyNumberNEW(em, addedEmergencyNumbers,
					cacheAddedEmergencyNumberObjectList, savedModification,
					cacheModification);
		}
	}

	public void checkInAddedEmergencyNumberNEW(
		...
		for (PCEmergencyNumber em : addedEmergencyNumbers) {
			if (em.getCountry().equals(emergencyNumber.getCountry())
					&& em.getDescription().equals(
							emergencyNumber.getDescription())
					&& em.getNumber().equals(emergencyNumber.getNumber())) {
				ArrayList<Key> modAddedEmergencyNumbers = savedModification
						.getAddedEmergencyNumbers();
				...
				savedModification
						.setAddedEmergencyNumbers(modAddedEmergencyNumbers);

				EmergencyNumberModel enm = new EmergencyNumberModel(
						KeyFactory.keyToString(em.getKey()), em.getCountry(),
						em.getNumber(), em.getDescription());
...
//						cacheModification, false);
				...
				if (!isActive) {
					...
					// already exists in smartphone inactive contact list. If
					// it's
					// inactive the contact is added to the inactive contact
					// list
...
//							cacheModification);
					...
					if (!isInactive) {
						...
						newContact = new PCSimpleContact(mc.getFirstName(),
								mc.getLastName(), phone.getKey());
						...
						newCacheContact.setKeyId(KeyFactory
								.keyToString(newContact.getKey()));
...
//						cacheModification, true);
	...
	public void saveInternalModificationsOLD(
			InternalModificationsModel internalModsModel)
			throws SessionQueryException, IllegalArgumentException {
		PersistenceManager pm = ServiceUtils.PMF.getPersistenceManager();

		try {
			logger.info("[Internal Modifications Service] Converting id: "
					+ internalModsModel.getId() + " of smartphone to Key.");
			Key smartphoneKey = KeyFactory.stringToKey(internalModsModel
					.getId());
			...
			// savedSmartphone.setLocation(internalModsModel.getLocation().convertToGeoPt());
			logger.info("[Internal Modifications Service] Setting location "
					+ internalModsModel.getLocation());
			if (internalModsModel.getLocation() != null) {
				GeoPt geopt = LocationModelUtils
						.convertToGeoPt(internalModsModel.getLocation());
				...
				if (savedSmartphone.getRoutes() != null) {
					...
					if (size > 0) {
						...
						Key routeKey = savedSmartphone.getRoutes()
								.get(size - 1);
						PCRoute route = pm.getObjectById(PCRoute.class,
								routeKey);
			...
			if (internalModsModel.getModification().getActiveContacts() != null
				...
				if (internalModsModel.getModification().getActiveContacts()
						.size() > 0) {
					logger.info("Checking added Contacts");
					checkAddedContactsOLD(internalModsModel.getModification()
							.getActiveContacts(),
							savedSmartphone.getActiveContacts(),
							savedSimpleActiveContactsObjectList,
							savedActiveContactsObjectList,
							savedSimpleInactiveContacsObjectList,
							savedInactiveContactsObjectList, modification);
				}

				if (internalModsModel.getModification().getInactiveContacts()
						.size() > 0) {
					logger.info("Checking deleted contacts");
					checkDeletedContactsOLD(internalModsModel.getModification()
							.getInactiveContacts(),
							savedSimpleActiveContactsObjectList,
							savedActiveContactsObjectList,
							savedSimpleInactiveContacsObjectList,
			...
			if (internalModsModel.getModification().getAddedEmergencyNumbers() != null
				...
				if (internalModsModel.getModification()
						.getDeletedEmergencyNumbers().size() > 0) {
					logger.info("Checking deleted emergency Contacts");
					checkDeletedEmergencyNumbersOLD(internalModsModel
							.getModification().getDeletedEmergencyNumbers(),
							savedSmartphone.getAddedEmergencyNumbers(),
							modification);
				}
			...
			logger.severe("[Internal Modifications Service] An error ocurred while finding the PCSmartphone by key "
					+ ex.getMessage());
			throw ex;
		} catch (NullPointerException ex) {
			logger.severe("[Internal Modifications Service] A null pointer exception ocurred "
					+ internalModsModel
					+ " "
					+ internalModsModel.getModification()
					+ " "
	...
	public void checkDeletedEmergencyNumbersOLD(
			ArrayList<EmergencyNumberModel> internalModDeletedEmergencyNumbers,
			ArrayList<Key> savedAddedEmergencyNumbers,
		...
		for (Key key : savedAddedEmergencyNumbers) {
			auxAddedEmergencyNumber = (PCEmergencyNumber) pm.getObjectById(
					PCEmergencyNumber.class, key);
			addedEmergencyNumbers.add(auxAddedEmergencyNumber);
		}

		for (EmergencyNumberModel em : internalModDeletedEmergencyNumbers) {
			checkInAddedEmergencyNumberOLD(em, addedEmergencyNumbers,
					modification);
		}
	}

	public void checkInAddedEmergencyNumberOLD(
			EmergencyNumberModel emergencyNumber,
			ArrayList<PCEmergencyNumber> addedEmergencyNumbers,
			PCModification modification) {
		for (PCEmergencyNumber em : addedEmergencyNumbers) {
			if (em.getCountry().equals(emergencyNumber.getCountry())
					&& em.getDescription().equals(
							emergencyNumber.getDescription())
					&& em.getNumber().equals(emergencyNumber.getNumber())) {
				ArrayList<Key> modAddedEmergencyNumbers = modification
						.getAddedEmergencyNumbers();
	...
	public boolean isActiveContactOLD(String firstName, String lastName,
			String phoneNumber, ArrayList<PCSimpleContact> savedSimpleContacts,
			ArrayList<PCPhone> savedActiveContacts,
			PCModification modification, boolean specialCase) {
		PersistenceManager pm = ServiceUtils.PMF.getPersistenceManager();
		boolean isActive = false;
		PCSimpleContact auxSimpleContact;
		int counter = 0;
		...
		for (PCPhone auxPhone : savedActiveContacts) {
			...
			if (auxSimpleContact.getFirstName().equals(firstName)
				...
				if (specialCase) {
					ArrayList<Key> modActiveContacts = modification
							.getActiveContacts();
	...
	public boolean isInactiveContactOLD(String firstName, String lastName,
			String phoneNumber, ArrayList<PCSimpleContact> savedSimpleContacts,
			ArrayList<PCPhone> savedInactiveContacts,
			PCModification modification) {
		PersistenceManager pm = ServiceUtils.PMF.getPersistenceManager();
	...
	public void checkAddedContactsOLD(
		...
		for (SimpleContactModel mc : internalModAddedContacts) {
			...
			for (PhoneModel p : phones) {
				...
				// the contact is active.
				// ------------------------------------------------------------
				isActive = isActiveContactOLD(mc.getFirstName(),
						mc.getLastName(), p.getPhoneNumber(),
						savedSimpleActiveContactsObjectList,
						savedActiveContactsObjectList, modification, false);
				...
				if (!isActive) {
					...
					// of the modifiction passed as parameter.
					// ------------------------------------------------------------
					isInactive = isInactiveContactOLD(mc.getFirstName(),
							mc.getLastName(), p.getPhoneNumber(),
							savedSimpleInactiveContacsObjectList,
							savedInactiveContactsObjectList, modification);
	...
	public void checkDeletedContactsOLD(
			ArrayList<SimpleContactModel> internalModDeletedContacts,
			ArrayList<PCSimpleContact> savedSimpleActiveContactsObjectList,
			ArrayList<PCPhone> savedActiveContactsObjectList,
			ArrayList<PCSimpleContact> savedSimpleInactiveContacsObjectList,
		...
		for (SimpleContactModel mc : internalModDeletedContacts) {
			...
			for (PhoneModel p : phones) {
				...
				// contact list.
				// ------------------------------------------------------------
				isActiveContactOLD(mc.getFirstName(), mc.getLastName(),
						p.getPhoneNumber(),
						savedSimpleActiveContactsObjectList,
						savedActiveContactsObjectList, modification, true);
			}
		}
	...
	// public boolean isActiveContact(String firstName, String lastName, String
	// phoneNumber, ArrayList<Key> savedActiveContacts, PCModification
	// modification, boolean specialCase) {
	// PersistenceManager pm = ServiceUtils.PMF.getPersistenceManager();
	// boolean isActive = false;
	// PCSimpleContact auxSimpleContact;
	// PCPhone auxPhone;
	...
	// auxSimpleContact =
	// (PCSimpleContact)pm.getObjectById(PCSimpleContact.class, c);
	// auxPhone = (PCPhone)pm.getObjectById(PCPhone.class,
	// auxSimpleContact.getPhone());
	//
	// if (auxSimpleContact.getFirstName().equals(firstName) &&
	// auxSimpleContact.getLastName().equals(lastName) &&
	// auxPhone.getPhoneNumber().equals(phoneNumber)) {
	...
	// public boolean isInactiveContact(String firstName, String lastName,
	// String phoneNumber, ArrayList<Key> savedInactiveContacts, PCModification
	// modification) {
	// PersistenceManager pm = ServiceUtils.PMF.getPersistenceManager();
	// boolean isInactive = false;
	// PCSimpleContact auxSimpleContact;
	...
	// public void checkAddedContacts(ArrayList<SimpleContactModel>
	// internalModAddedContacts, ArrayList<Key> savedActiveContacts,
	// ArrayList<Key> savedInactiveContacts, PCModification modification) {
	// PersistenceManager pm = ServiceUtils.PMF.getPersistenceManager();
	//
	// boolean isActive;
	// boolean isInactive;
	...
	// // the contact is active.
	// //------------------------------------------------------------
	// isActive = isActiveContact(mc.getFirstName(), mc.getLastName(),
	// p.getPhoneNumber(), savedActiveContacts, modification, false);
	//
	// if (!isActive) {
	// logger.info("[Internal Modifications Service] Contact is not active.");
	// //------------------------------------------------------------
	...
	// // of the modifiction passed as parameter.
	// //------------------------------------------------------------
	// isInactive = isInactiveContact(mc.getFirstName(), mc.getLastName(),
	// p.getPhoneNumber(), savedInactiveContacts, modification);
	//
	// if (!isInactive) {
	// logger.info("[Internal Modifications Service] Contact is not inactive.");
	// //------------------------------------------------------------
	...
	// pm.makePersistent(phone);
	//
	// newContact = new PCSimpleContact(mc.getFirstName(), mc.getLastName(),
	// phone.getKey());
	//
	// pm.makePersistent(newContact);
	//
	...
	// public void checkDeletedContacts(ArrayList<SimpleContactModel>
	// internalModDeletedContacts, ArrayList<Key> savedActiveContacts,
	// ArrayList<Key> savedInactiveContacts, PCModification modification) {
	// PersistenceManager pm = ServiceUtils.PMF.getPersistenceManager();
	//
	// logger.info("[Internal Modification Service] Searching for contacts from keys.");
	...
	// // contact list.
	// //------------------------------------------------------------
	// isActiveContact(mc.getFirstName(), mc.getLastName(), p.getPhoneNumber(),
	// savedActiveContacts, modification, true);
	// }
	// }
	//
	// pm.close();
